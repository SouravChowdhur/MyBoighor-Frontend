...
const Users = () => {
  ...
  <main className='relative flex-1 p-6 pt-28 '>
       ...
       <header className='flex flex-col gap-3 md:flex-row md:justify-between md:items-center'>
        <h2 className='text-xl font-medium md:text-2xl md:font-semibold text-orange-500'>Registered Users</h2>
       </header>


       {
        users && users.filter(u=> u.role === "User").length >0 ? (
           <div className='mt-6 overflow-auto bg-orange-100 rounded-md shadow-lg'>
                  ...
                  
                    {
                      users.filter(u=> u.role === "User").map((user, index)=>(
                            <tr key={user._id} className={(index+1)%2 === 0 ? "bg-gray-50":""}>
                                    ...
                                    <td className='px-4 py-2'>{user.createdAt ? formatData(user.createdAt) : "N/A"}</td>
                            </tr>
                      ))
                    }
                  </tbody>
